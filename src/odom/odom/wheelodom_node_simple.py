...
- Subscribes to four wheel encoder pulse counts (Int32)
- Subscribes (optionally) to IMU for orientation fusion
- Computes robot pose (x, y, θ) and velocity (vx, vy, ω)
- Publishes nav_msgs/Odometry on '/odom' and broadcasts TF base_link→odom
- Fuses encoder‐based heading with IMU orientation via quaternion blending
"""
...
class MecanumOdometryNode(Node):
    def __init__(self):
        ...
        self.create_subscription(
            Int32,
            '/encoder/pulse_count_1',
            self.encoder_callback_1,
            qos_best_effort)
        self.create_subscription(
            Int32,
            '/encoder/pulse_count_2',
            self.encoder_callback_2,
            qos_best_effort)
        self.create_subscription(
            Int32,
            '/encoder/pulse_count_3',
            self.encoder_callback_3,
            qos_best_effort)
        self.create_subscription(
            Int32,
            '/encoder/pulse_count_4',
    ...
    def imu_callback(self, msg):
        ...
        try:
            ...
            # 从 transform 提取旋转四元数
            transform_quaternion = [transform.transform.rotation.x,
            ...
            if self.initial_base_link_quaternion is None:
                self.initial_base_link_quaternion = self.base_link_quaternion
                self.get_logger().info(
                    f"Initial base_link quaternion: {self.initial_base_link_quaternion}")

            # 计算从 transform_quaternion 得到的旋转矩阵
            rotation_matrix = tf_transformations.quaternion_matrix(transform_quaternion)[
                :3, :3]
            ...
            self.get_logger().warn(
                f"Transform warning at time {msg.header.stamp}: {e}")
    ...
    def get_relative_quaternion(self):
        ...
        vector_norm = np.linalg.norm(
            relative_quaternion[:3])  # 向量部分 (x, y, z) 的模量
        threshold = 1e-3  # 阈值，可根据实验调整
        if vector_norm < threshold:
            relative_quaternion[:3] = [0.0, 0.0, 0.0]  # 将向量部分归零
            relative_quaternion[3] = 1.0  # 四元数归一化，w = 1 表示无旋转
    ...
    def update_wheel_speed(self, wheel_index, current_pulse):
        ...
        dt = (current_time -
              self.last_time_encoders[wheel_index]).nanoseconds / 1e9
    ...
    def pulse_to_rpm(self, delta_pulse, dt):
        ...
        angular_velocity = (pulse_rate / self.encoder_ppr) * \
            2 * np.pi  # 转为角速度 (rad/s)
    ...
    def update_odometry(self):
        ...
        assert velocity_matrix.shape == (
            3, 4), "Velocity matrix dimensions must be 3x4."
