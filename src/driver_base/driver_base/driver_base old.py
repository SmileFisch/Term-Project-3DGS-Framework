"""
Encoder and Motor Control for Mecanum Robot with ROS 2 Integration
- Encoder: subscribes to wheel encoder pulse count topics, computes real‐time linear speed
- Motor: wraps PWM, H‑bridge direction, optional encoder feedback, PID velocity control
- CarController: provides differential, axial, and mecanum drive algorithms using four Motor instances
- main(): initializes nodes, motors, and encoders; runs a 5 s motor test while logging speeds
"""
...
class Encoder:
    """
    Encoder class, automatically creates a ROS 2 node and subscribes to the relevant topic.
    """

    def __init__(self, enc_a_pin, enc_b_pin):
        """
        Initializes the encoder class and automatically creates a node.
        :param enc_a_pin: A phase pin number
        :param enc_b_pin: B phase pin number
        """
        # Initialize the ROS 2 system (ensure it's only called once)
        if not rclpy.ok():
            rclpy.init()

        # Dynamically create a node
        self.node = rclpy.create_node(f"encoder_node_{enc_a_pin}_{enc_b_pin}")

        # Dynamically select the topic
        self.topic_name = self._select_topic(enc_a_pin, enc_b_pin)
        if not self.topic_name:
            raise ValueError(f"No topic mapped for enc_a_pin={enc_a_pin}, enc_b_pin={enc_b_pin}")
    ...
    def _select_topic(self, enc_a_pin, enc_b_pin):
        """
        Select the topic based on A/B phases.
        """
        encoder_configs = {
    ...
    def _pulse_count_callback(self, msg):
        """
        Callback function for pulse count.
        """
        self.node.get_logger().info(f"Received pulse count: {msg.data}")
        self.pulse_count = msg.data

    def get_speed(self, wheel_diameter=0.055, ticks_per_revolution=260, gear_ratio=20):
        """
        Calculate the speed based on pulse count.
        ...
        # self.node.get_logger().info(f"Current time: {current_time}, Last time: {self.last_time}, Elapsed time: {elapsed_time}")

        if elapsed_time == 0:
            return self.last_speed

        delta_pulse = self.pulse_count - self.previous_pulse_count
        # self.node.get_logger().info(f"Pulse Count: {self.pulse_count}, Previous Pulse Count: {self.previous_pulse_count}, Delta Pulse: {delta_pulse}")
        self.previous_pulse_count = self.pulse_count

        revolutions = delta_pulse / (ticks_per_revolution * gear_ratio)
...
class Motor:
    """
    Motor encapsulation class, includes PWM, direction control, encoder, and PID control logic.
    """

    def __init__(self, pwm_pin, dir1_pin, dir2_pin, encoder=None, compensate_rate=1):
    ...
    def set_duty_cycle(self, target_speed, max_rpm, wheel_diameter, gear_ratio):
        """
        Set the duty cycle based on target speed.
        """
    ...
    def update(self, target_duty_cycle, wheel_diameter, ticks_per_revolution, gear_ratio, max_rpm):
        ...
        """
        if not self.encoder:
            # Open-loop control logic when no encoder is available
            self.set_speed(target_duty_cycle)
            return

        # Get current speed (m/s)
        current_speed = self.get_speed(wheel_diameter, ticks_per_revolution, gear_ratio)
        ...
        output_speed = self.kp * error + self.ki * self.integral + self.kd * derivative
    ...
    def set_speed(self, speed):
        ...
        """
        speed = max(-1.0, min(1.0, speed * self.compensate_rate))  # Limit speed range
        if speed > 0:  # Forward
            self.dir1.on()
            self.dir2.off()
    ...
    def get_speed(self, wheel_diameter, ticks_per_revolution, gear_ratio):
        """
        Get the linear speed of the motor.
        """
        if self.encoder is not None:
            return self.encoder.get_speed(wheel_diameter, ticks_per_revolution, gear_ratio)
        else:
            self.get_logger().info("No Encoder!!!!")
            return 0
...
class CarController:
    ...
    def mecanum_drive(self, x, y, xr, wheel_base, wheel_track):
        ...
        rear_left_duty_cycle = self.motors[1].speed_to_duty_cycle(
            rear_left_speed, self.max_rpm, self.wheel_diameter, self.gear_ratio
        )
        front_right_duty_cycle = self.motors[2].speed_to_duty_cycle(
...
def main():
    ...
    ticks_per_revolution = 4600  # Pulses per revolution
    ...
    motors = [
        Motor(pwm_pin=12, dir1_pin=1, dir2_pin=27, encoder=Encoder(enc_a_pin=25, enc_b_pin=17)),
        # lf
        Motor(pwm_pin=19, dir1_pin=20, dir2_pin=16, encoder=Encoder(enc_a_pin=26, enc_b_pin=21)),
        # lb
        Motor(pwm_pin=18, dir1_pin=14, dir2_pin=15, encoder=Encoder(enc_a_pin=23, enc_b_pin=24)),
        # rf
        Motor(pwm_pin=13, dir1_pin=6, dir2_pin=5, encoder=Encoder(enc_a_pin=0, enc_b_pin=11)),
        # rb
    ]
